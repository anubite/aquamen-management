...
function TransactionImport({ isOpen, onClose, token, onComplete }) {
                    ...
                    {!activeImport ? (
                        <>
                            <div
                                ...
                                onDrop={handleDrop}
                            >
                                <input type="file" id="tx-file-upload" accept=".xlsx" onChange={handleFileChange} hidden />
                                <label htmlFor="tx-file-upload" className="upload-label">
                                    <FileText size={48} color="var(--primary)" />
                                    <span>{file ? file.name : 'Select or drop internet banking Excel file'}</span>
                                </label>
                                <button
                            ...
                            <div className="history-zone">
                                ...
                                <div className="history-list">
                                    {importHistory.map(imp => (
                                        <div key={imp.id} className="history-item" onClick={() => { setActiveImport(imp); setLogs([]); }}>
                        ...
                        <div className="active-import">
                            <div className="import-status-header">
                                ...
                                <button className="btn btn-sm" onClick={() => { setActiveImport(null); setLogs([]); }}>Back to Upload</button>
                            </div>

                            <div className="progress-bar-container">
                                <div className={`progress-bar ${activeImport.status}`} style={{ width: activeImport.status === 'completed' || activeImport.status === 'failed' ? '100%' : '50%' }}></div>
                            </div>

                            <div className="log-container">
                                {logs.length === 0 && <div style={{ padding: '1rem', textAlign: 'center', color: 'var(--text-muted)' }}>Waiting for logs...</div>}
                                {logs.map(log => (
